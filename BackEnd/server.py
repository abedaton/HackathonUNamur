...
class Server:
    ...
    def addUser(self, username, ip)->bool:
        if self.checkIfOk(username):
            with self.conn:
                self.cursor.execute("INSERT INTO User_Info VALUES (:username, :ip)", {'username': username, 'ip': ip})
            print("Utilisateur '{} ({})' ajouté".format(username, ip))
            return True
        else:
            print("L'utilisateur '{} ({})' existe déjà!".format(username, ip))
            return False

    def checkIfOk(self, username)->bool:
        self.cursor.execute("SELECT * FROM User_Info WHERE username = :username", {'username': username})
